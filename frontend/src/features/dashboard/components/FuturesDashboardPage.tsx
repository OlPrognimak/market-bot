...
export function FuturesDashboardPage({ token, currentUser }: { token: string; currentUser: AppUser }) {
  ...
  const results = useMemo(() => (snapshot?.results ?? []).filter(r => [r.symbol, r.name, r.underlying, r.region].join(" ").toLowerCase().includes(query.toLowerCase())), [query, snapshot]);
  ...
  return <main className="dashboard">
    ...
    <div className="table-frame market-results-frame"><table className="market-results-table futures-results-table"><thead><tr><th>Contract</th><th>Underlying</th><th>Region</th><th>Price</th><th>From Settlement</th><th>From Open</th><th>Delta</th><th>Rolling</th><th>Freshness</th><th>Updated</th></tr></thead>
